...
public class se3ops
{
		...
		if (se3alg.matrix.getSubMatrix(0, 0, 3, 3).isSkewSymmetric())
		{
			if (se3alg.matrix.getSubMatrix(3, 0, 1, 4).isEqual(Matrix.zeros(4, 4).getSubMatrix(3, 0, 1, 4)))
			{
				...
				if (omg.norm() == 1)
				{
					...
					log+="result1st="+result.rows+"x"+result.cols+"\n";
				...
				else if (vel.norm() == 1.0 && omg.norm() == 0)
				{
                    ...
                    log+="se3g2="+se3g.matrix.toString();
				}
				else
				{
	...
	public static se3algebra matrixLog6(se3group se3gr,Boolean verbose)
	{
        log="";
		...
		{   
           ...
           // Vector3 omgvec= new Vector3(se3gr.matrix.getSubMatrix(0, 0, 3, 3));
			...
			{
                log+="+++++\n";
				so3algebra so3alg = so3ops.matrixLog3((new so3group(se3gr.matrix.getSubMatrix(0, 0, 3, 3))),false);
                ...
                log+="matrixlogres="+se3alg.matrix.toString();
                MainActivity.appendLog(log,verbose);
                return se3alg;
			}
            }
